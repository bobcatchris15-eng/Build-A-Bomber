...
**7.1 Tilt-shift exists only in the Design Lab.** [designer_camera.gd](prototype/scripts/designer_camera.gd) implements the full near/far DOF band; [rts_camera.gd](prototype/scripts/rts_camera.gd) has no DOF at all. The battlefield — where the player spends nearly all their time and where the miniature read matters most — currently has no macro optics. This is the largest gap between this document and the build, and probably the highest-value single change in it.
...
**7.5 Existing weapon audio is conventional.** `sfx_cannon`, `sfx_machine_gun`, `sfx_laser`, `sfx_missile` and `sfx_explosion` are all in the roster and are presumably real recordings. §6 requires all five to be re-recorded as vocalisations. That is a real content task with a real tonal risk, and it is worth prototyping on a *single* weapon and playtesting before committing the whole set.
