# Locomotion — Expansion & Systems Upgrade Plan
...
1. **Do the systems pass (Phase 0/1) first, as a standalone commit**, before any
   new locomotion type is authored. — APPROVED
2. **7 new locomotion types** (10 → 17) is the agreed scope. — APPROVED
3. **Rearchitect the placement system** for robustness/flexibility — added
   2026-08-01, see §2. Findings verified against source; recommendation is to
...
describes the *rebuild* — turning each monolithic locomotion `.glb` into a
...
`Arsenal_Weapons_List.md` — same method: survey, close the systemic gaps first,
...
|---|---|---|---|---|
| `wheels` | ground_contact, **high_speed** | 350 | (default) | ✅ |
| `tracked_treads` | ground_contact | 700 | (default) | ✅ |
| `legs` | ground_contact | 500 | (default) | ✅ |
| `hover_engine` | hovering | 300 | (default) | ❌ |
| `helicopter_rotors` | airborne, **rotary_wing**, hovering | 250 | (default) | ❌ |
| `fixed_wing_engine` | airborne, fixed_wing, **high_speed** | 380 | (default) | ❌ |
| `ornithopter_wing` | airborne, flapping_wing | 300 | 120 | ❌ |
| `buoyant_envelope` | airborne, buoyant | 1100 | 55 | ❌ |
| `naval_propeller` | buoyant, naval | 800 | (default) | ❌ |
| `screw_drive` | ground_contact, amphibious | 600 | 110 | ✅ |
...
**G1 — `TERRAIN_SPEED_MULTIPLIERS` covers 4 of 10 types.**
...
The other six fall back to `1.0` on every surface — they have **no terrain
...
**G2 — dead traits.** `high_speed` and `rotary_wing` are declared in the
...
delete them — right now they read like implemented behaviour and are not.

**G3 — `get_locomotion_contribs()` returns `capacity = 0.0` for five types**
...
**G4 — tweak depth is uneven, and one pair is a literal duplicate.**
...
byte-identical spec (documented as deliberate at the time — shared pylon-prop
rebuild — but it means picking between them is a stats choice with no design
choice attached).
...
(`rotor_blade`)**. Also heavy: `screw_drum` ×3 variants @ 1,596 each,
...
how instanced the part is — `wheel_hub` at 656 is drawn 4–48 times per unit;
`rotor_blade` at 12 is drawn 2–32. This is the same uneven-detail-standard
...
## 2. Architecture review — the placement system
...
Placement lives in **`module_placer.gd:676-1219`, `update_locomotion()`** — a
single ~540-line function that is, structurally, an
`if type_id == "wheels": … elif type_id == "tracked_treads": …` chain with one
...
Steps 4–6 are near-identical in all ten branches and are copy-pasted ten times.
Step 2 is the only genuinely per-type part.

### 2.2 The five architectural problems

**A1 — Adding a type means editing a 540-line function.** Seven new types
(§4) means seven more `elif` branches, taking `update_locomotion()` past 800
lines. There is no registration point, no data table, no interface — the
...
**A2 — Collider geometry math is written three times and has already drifted.**
...
in-code comments document this drifting before — "needing to be clicked very
...
formula, kept in sync by hand, is the defect generator here** — and only three
...
**A3 — The "reach vector" convention is real but unspelled.** Six types tell
...
| `wheels`, `tracked_treads`, `legs`, `ornithopter_wing` | none — plus one-off keys like `leg_stance_reach`, `leg_hull_centerline_y`, `target_length` |

All of it travels as untyped floats in a `Dictionary`, so a typo'd key is a
silent default, not an error. This is one concept — *"here is the vector from my
origin back to the hull's center"* — that deserves one name and one shape.

**A4 — Nothing re-runs the layout when the hull changes.** `update_locomotion()`
...
`gizmo_3d.gd:210-260` handles hull rescale — it updates `hull_scale`, the mesh,
...
`z_limit`, `drum_length`, the ellipse radii…), resizing a hull after choosing
...
**A5 — The blueprint round-trip bakes the layout instead of re-deriving it.**
`blueprint_manager.gd` saves locomotion instances as ordinary modules with baked
`position`/`rotation` (`:733+`), and on load rebuilds them generically —
...
drifted — it reads `settings.get("size", 1.0)` where the placer reads
`settings.get("wheel_size", …) * hull_height_factor`, and `size` is no longer a
...
files** — but it is exactly the shape of drift that A2 keeps producing, and it
...
The shape that fits what's actually there — not a rewrite, a **factoring**. The
...
the layout → clear old parts → build running gear if wanted → ask the layout for
`Array[MountStation]` → loop, calling the same `_place_weapon()` + reset +
mirror + append it does now, **once**. The ten copies of steps 4–6 collapse to
one. Target: ~120 lines total, down from 540.

**(3) A typed `MountStation`.** `{ position: Vector3, normal: Vector3,
reach: Vector3, index: int, side: float, mirror: bool }` — replacing the
...
**(5) `refresh_locomotion()` — idempotent re-layout.** A no-argument entry point
...
### 2.4 Why this is worth doing before §4, not after

Adding seven types to the current structure means writing seven more copies of
steps 4–6, seven more `geo_tweaks` dictionaries in whatever spelling seems right
that day, and — since only three of ten types have collider overrides today —
...
- `test_every_locomotion_type_has_a_layout()` — every catalog locomotion id
  resolves to a `LocomotionLayout`; fails loudly for a type added without one.
- `test_locomotion_layout_is_deterministic_and_hull_relative()` — lay out each
  ...
  coincide.
- `test_locomotion_colliders_match_their_meshes()` — for every type and a
...
- `test_hull_rescale_relays_out_locomotion()` — place wheels, rescale the hull
  2×, assert the outermost station moved. Pins A4.
- `test_locomotion_survives_a_blueprint_round_trip()` — save/load a design of
  each type, assert station positions match the freshly-placed ones. Pins A5.

### 2.6 Risks specific to the rearchitecture

- **R5 — behavioural drift during the factoring.** Ten branches carry a lot of
...
**0.1 — Fill out `TERRAIN_SPEED_MULTIPLIERS` to all 10 types.** Proposed rows,
...
"fixed_wing_engine":  all 1.0                               (flat by design — document it)
"ornithopter_wing":   all 1.0 EXCEPT forest 0.9
"buoyant_envelope":   all 1.0                               (flat by design)
"naval_propeller":    n/a on land — see 0.2
```
...
numbers — ground-effect lift ignoring marsh/snow but hating rocky is the whole
character of the thing and it currently has none.

**0.2 — Decide the naval/land question.** `naval_propeller` never touches the
...
intentional rather than incidental. Prefer the explicit entry — cheaper, and it
makes the table a complete inventory.

**0.3 — Trait cleanup.** Three options per dead trait; recommendation in bold:
- `high_speed` → **wire it**: a flat speed bonus + a turn-rate penalty in
  `battle_unit.gd`, so "fast but can't corner" becomes a real tradeoff that
  `wheels` and `fixed_wing_engine` share.
- `rotary_wing` → **wire it** to the existing rotor-spin animation branch in
  `battle_unit.gd` (which currently does a by-name `get_node_or_null()` lookup —
  see R1 in the rebuild plan). Trait-driven is more robust than name-driven.
- `flapping_wing` → **keep as pure flavour**, but comment it as such.

**0.4 — Give the five capacity-less types a capacity term** in
...
**0.5 — Differentiate `naval_propeller` vs `buoyant_envelope` tweaks.** Keep
the shared pylon-prop geometry (that rebuild was right), but add one
type-unique slider each: naval gets **`kort_nozzle` (bool)** — thrust up,
top speed down; envelope gets **`envelope_volume`** — capacity up, thrust
efficiency down. One slider each is enough to make them distinct decisions.

**0.6 — Tests.** Add to `prototype/run_tests.gd`:
- `test_every_locomotion_type_has_terrain_character()` — iterate all catalog
  ...
  allowlist.
- `test_every_declared_locomotion_trait_is_consumed()` — collect traits from
  the catalog, grep-equivalent assert each appears in a consumer set. Prevents
  G2 recurring.
- `test_every_locomotion_tweak_moves_a_stat()` — for each type, for each tweak,
...
## 4. Phase 1 — Roster expansion (10 → 17)
...
**`half_track`** — the explicit compromise slot. Wheels' gravel/road speed with
...
**`rocker_bogie`** — the terrain specialist. Slow everywhere, near-immune to
...
- parts: `rg_*` prefixed parts already exist in the asset list — check reuse
  before authoring.

### Hover

**`air_cushion_skirt`** — a real hovercraft, not a sci-fi pad. Big footprint,
...
**`anti_grav_plate`** — the crystal-expensive terrain-agnostic option. Flat 1.0
...
**`hydrofoil`** — fast, poor capacity, high draught penalty so it can't work
...
**`water_jet`** — the inverse: shallow-capable, high thrust, efficiency falls
...
**`pontoon_wheels`** — a light counterpart to the heavy `screw_drive`. Cheap,
...
## 5. Phase 2 — Mesh consistency pass

Runs after Phase 1, or interleaved per-type. Not urgent — `ff757ef`'s LOD
...
roster *up*, not down. The old framing — decimate the heavy parts to a budget —
...
- **Leave the heavy parts alone** unless the density is bevel-segment bulk that
  buys nothing at silhouette scale — the `bevel_segments=1` check from the armor
  greeble pass, applied as a *quality* question rather than a budget one.
- **Watch instance counts, not totals.** `wheel_hub` is drawn 4–48× per unit and
  `rotor_blade` 2–32×; detail added there multiplies. The LOD regeneration
  handles distance, so the constraint is authoring time and close-up read, but
  a part instanced 48× still deserves more scrutiny than a one-off gearbox.
...
1. **Commit A — systems pass.** Phase 0 entire (0.1–0.6). No new types, no new
   ...
   lake-pathfinding suite flakes under CPU contention — kill stale Godot
   processes before judging a run).
2. **Commit B — golden-layout fixture.** §2.5's station-position test written
   ...
   must land first (R5).
3. **Commit C — placement factoring.** §2.3 items (1)(2)(3): `LocomotionLayout`,
   the generic loop, `MountStation` + the loose-key shim. Commit B's golden
   values must reproduce **exactly** — no visual changes ride along.
4. **Commit D — collider sourcing + re-layout.** §2.3 items (4)(5): colliders
...
5. **Commit E — ground pair.** `half_track`, `rocker_bogie` + tests. First real
   exercise of the new layout data path — if either needs a code change rather
   than a data declaration, the factoring missed a pattern; fix it here.
6. **Commit F — hover pair.** `air_cushion_skirt`, `anti_grav_plate` + tests.
7. **Commit G — naval pair.** `hydrofoil`, `water_jet` + tests. Includes the
   draught interaction, which is the riskiest single item in the plan.
8. **Commit H — `pontoon_wheels`** + tests.
9. **Commit I — mesh consistency pass** + `PROGRESS.md` catch-up.
...
- **R1 — navmesh count.** `air_cushion_skirt` and `pontoon_wheels` both want
...
- **R2 — draught.** `hydrofoil` is the first locomotor that would be *excluded*
...
- **R3 — 17 types vs UI.** `parts_menu.gd` groups locomotion into 4 buckets.
...
- **R4 — balance drift.** Phase 0.4 adds capacity to five types that had none;
