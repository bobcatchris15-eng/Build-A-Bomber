...
**[Claude]** or **[Qwen]** — see `PERFORMANCE_PLAN.md`'s delegation section
for how that split works in practice; the same rules apply here.

## Non-scope — read this before adding anything below

This project already has two other planning documents that cover most of
"make the game look/feel better." **Do not re-plan these here** — finish
them instead:

- **`VISUAL_IMPROVEMENT_PLAN.md`** — the 2D UI chrome overhaul brief (theme,
  ...
  already be built — `resources/bomber_theme.tres`, `scripts/ui_icons.gd`,
  ...
  `ui_anim.gd` motion library) are still unbuilt — see this doc's Part A for
  a couple of things worth doing alongside F, but F/G's actual specs live in
  `VISUAL_IMPROVEMENT_PLAN.md`, not here.
- **`RTS_CORE_ROADMAP.md`** — minimap (**B9**), the categorised build bar +
...
- **`FABLE_REVIEW.md`** — flags that the project's real "build diversity"
  problem is combat-balance math (rapid-fire weapons deal zero damage to
  armored hulls — a threshold/per-shot-damage mismatch), not the visuals or
...
their **UX/readability convention** — legible health bars, clear selection
feedback, a minimap, control groups, order markers — which is Part C below
...
## Part A — 3D rendering fidelity

- **A1. Environment/post-processing pass** — *afternoon* [Qwen once Claude
  sets exact parameter values]. No `WorldEnvironment` tuning found anywhere
  in the project. Add: glow/bloom (existing emissive parts — energy shields
  via `shield_mode`, engine glow, muzzle flashes — have emission data
  ...
  visual-return-for-effort item — it's one `WorldEnvironment` node with a
  handful of tuned values, not new geometry or systems.
- **A2. Replace ad-hoc VFX with `GPUParticles3D`** — *one session*
  ...
  then auto-cleanup via `finished` signal or a lifetime timer) — visually
...
- **A3. Activate the decal system** — *multi-session* [Claude]. The
  ...
  stripes, faction insignia, battle-damage scorch marks) — this is real art
  production, not a code task, so budget it like `RTS_CORE_ROADMAP.md`'s B8
  (its own arc, not a quick chunk).
- **A4. Finish `VISUAL_IMPROVEMENT_PLAN.md` chunk F** — *one session*
  [Claude]. In-world health bars are still `Label3D` text rendering an
  `■□` ASCII bar (`battle_unit.gd:601-621`, `building.gd:255-262`,
  `target_dummy.gd:44` — three independent duplicated implementations of
  ...
  numbers) — this is pure execution against an existing spec, no new
  design needed here.

## Part B — Camera & input feel

- **B1. RTS camera polish** — *afternoon* [Qwen once Claude picks the
  easing curves/constants]. `rts_camera.gd` has WASD pan and scroll-zoom
  with only zoom height lerped (`:38`) — pan itself is un-eased, there's no
...
- **B2. Design Lab orbit camera smoothing** — *small* [Qwen once Claude
  picks constants]. `designer_camera.gd:40-46` applies zoom/orbit as direct
  `_distance -=` / `position.z =` snaps — no lerp at all, unlike the RTS
...
## Part C — UX gaps not tracked in any other doc

- **C1. Shift-click add-to-selection + control groups** — *one session*
  [Claude — input-handling correctness, not mechanical]. `skirmish.gd`'s
...
- **C2. Stop debug tooling from shipping live** — *small* [Qwen once
...
- **C3. Wire up `CursorManager`** — *afternoon* [Qwen once Claude specifies
  ...
  `skirmish.gd`, `battlefield.gd`, or `battle_unit.gd` — grep confirms the
  ...
  move/attack/harvest by raycast target) — this is "call an existing
  function from another existing function," not new design.
- **C4. Fix the drawer's fake animation** — *trivial* [Qwen, this session's
  delegation trial — see below]. `parts_menu.gd:159-178`'s
  `_toggle_drawer()` has a comment claiming "content slides down with
  smooth animation," but the function body only sets
  `content.visible = true/false` — an instant flip, no tween. Add the
  ...
  using if it lands first — otherwise a plain `Tween.tween_property` over
...
**This session's trial run is C4** (the drawer-tween fix) — it's small,
...
validate the invoke → review → test workflow before handing qwen anything
...
|---|---|---|---|---|---|
| 1 | A1 environment/post-processing | afternoon | — | Qwen (Claude-specified) | — |
| 2 | A2 GPUParticles3D VFX | one session | — | Qwen (Claude-specified) | — |
| 3 | A3 decal system | multi-session | — | Claude (art production arc) | — |
| 4 | A4 finish chrome-plan chunk F | one session | — | Claude | — |
| 5 | B1 RTS camera edge-scroll/zoom-to-cursor | afternoon | — | Qwen (Claude-specified) | — |
| 6 | B2 Design Lab camera smoothing | small | — | Qwen (Claude-specified) | — |
| 7 | C1 shift-select + control groups | one session | — | Claude | — |
| 8 | C2 gate debug tooling | small | — | Qwen (Claude-specified) | — |
| 9 | C3 wire up CursorManager | afternoon | — | Qwen (Claude-specified) | — |
| 10 | C4 drawer tween fix | trivial | — | **Qwen — this session's trial** | — |

No hard dependencies between these chunks — they're independent enough to
...
- UX chunks (C1-C4): manual play-test — these are exactly the class of bug
