extends Node3D
# Windowed capture of the roster-expansion weapons, one close-up per
...
var subjects = [
	...
	["autocannon", {}],
	["autocannon", {"barrel_length": 2.0, "drum_size": 1.8}],
	["recoilless_rifle", {}],
	["coil_gun", {}],
	["coil_gun", {"rail_length": 1.8}],
	["napalm_mortar", {}],
	["mine_layer", {"tube_count": 4.0}],
	["ballista", {}],
	["smoke_discharger", {"tube_count": 6.0}],
]
...
func _build(index: int):
	...
	plate.size = Vector3(1.6, 0.08, 1.6)
	deck.mesh = plate
	...
	var reach = max(bounds.size.x, max(bounds.size.y, bounds.size.z)) * 0.62 + 0.28
...
func _process(_delta):
	...
	if frame_count % 10 == 0 and shots_taken < subjects.size():
		...
		if not subjects[shots_taken][1].is_empty():
			label += "_tweaked"
		img.save_png("res://scratch/roster_%02d_%s.png" % [shots_taken, label])
		shots_taken += 1
