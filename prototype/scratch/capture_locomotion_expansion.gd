...
func _build(index: int):
	...
	# Locomotion hangs BELOW its mount point - struts, skirts, drums and plates
	# all project downward from the hull's underside - so on a deck plate at
	# y=0 most of these types render entirely inside it. Lift the assembly so
	# its lowest point rests on the plate, which is also how it reads in the
	# Lab once the hull is above it.
	holder.position.y = -bounds.position.y
...
func _process(_delta):
	...
	if frame_count % 10 == 0 and shots_taken < subjects.size():
		...
		if not subjects[shots_taken][1].is_empty():
			label += "_tweaked"
		img.save_png("res://scratch/loco_%02d_%s.png" % [shots_taken, label])
		shots_taken += 1
