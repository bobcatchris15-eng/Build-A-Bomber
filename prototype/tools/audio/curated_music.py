...
(never repeating consecutively) each time the current one finishes. Every
other state still gets exactly one file.
"""
...
LOOPING_STATES = {"menu", "lab", "operations"}   # single-track states that loop
# Skirmish is not in LOOPING_STATES: looping is handled by ROTATION (advancing
# to a new track), not by the AudioStream looping itself. See the module
# docstring and audio_manager.gd's _on_music_stem_finished.
