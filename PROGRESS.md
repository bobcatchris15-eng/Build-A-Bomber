...
**Autocannon remodel (`build_roster_expansion.py`).** The old model was a generic "long thin barrel with a pepper-pot brake", which is not what an M230 looks like at all. Rebuilt around what actually makes one recognisable: a *short*, squat, near-square receiver rather than a long rifle action; the **chain drive housing bulging off the left flank** as a flat oval cover with the sprocket bosses standing proud of it (the feature the whole weapon is named for, and the reason the silhouette is asymmetric); a **plain unbraked muzzle** — a chin gun firing forward over open ground has nothing to protect and a brake would blast the airframe; and the gun hung *inside* a U-shaped chin **yoke** with visible elevation actuators instead of sat on a pintle. The belt-fed side box became a linkless drum feeding up into the receiver's underside, which is where a chin gun's ammunition actually comes from. All four part origins are unchanged, so `visual_builder`'s existing assembly and the `caliber`/`barrel_length`/`drum_size` scaling needed no edits.

**Anti-materiel rifle — filling the roster's precision gap.** Every existing weapon is either DPS or splash; nothing was *precision*. This one is a single very large number every 4.5 seconds: **351 per shot, the largest direct-fire figure in the game**. That is the interesting place on the damage curve because per-shot damage (`dps × fire_rate`, since `fire_rate` is an interval) is exactly what `damage_resolver`'s armor thresholds gate on — so it clears every threshold in the table outright, and is comprehensively wasted on a scout it can only hit once every four and a half seconds. Slow to traverse, expensive in crystal, defenceless once something closes: an ambush and overwatch piece.

Six authored parts, split so each tweak moves exactly one of them: `amr_mount` (trunnion cradle with equilibrator spring stacks), `amr_breech` (long, heavily greebled, running back *through* the trunnions so the gun reads as balanced about its middle — Chris's specific note), `amr_barrel`, `amr_muzzle_brake` (its own part, so `barrel_length` *moves* the brake instead of stretching its baffles), `amr_sensor_pod` (the "expanded sensors": day sight, thermal aperture, laser rangefinder, met probe — sitting where another weapon would carry ammunition), and `amr_bipod`.

**The tweaks are the point.** `optic_power` buys **reach and nothing else** — explicitly not damage, or it stops being a trade and becomes a strictly-better slider — and is paid for in crystal (×1.6 per point) rather than metal or mass, because a better sight is not more steel. `bipod_deploy` is the roster's first tweak that buys a stat with a **capability** rather than with weight or cost: deployed, the rifle reaches 45% further and **cannot fire while its vehicle is moving**. That makes the slider a real question about how the vehicle will be used rather than a number you drag right, which is what `DESIGN_VISION.md` asks of a tweak. The bipod appears on the model only when down, so a deployed rifle is readable at a glance.

**Two plumbing traps, one of them nasty.** `bipod_deploy` ranges 0..1, so adding it to any of `module_data`'s blanket linear multiplier lists would make an *un*deployed bipod multiply the module's weight by **zero** and silently delete the design's mass. It is additive instead, and there is now a test that fails if anyone ever moves it. `optic_power` was initially dropped into those same lists and a 2× sight doubled the entire weapon's mass — nonsense, since the optic is a fraction of the gun — so it got its own branch: ×1.35 weight, ×1.20 metal, ×1.60 crystal.

**Two findings from my own test, both real.** The suite asserted the rifle had the largest per-shot number in the roster and it did not — artillery lands 405. The right claim is the largest *direct-fire* number: artillery buys its 405 with indirect fire it cannot aim at what is in front of it, and with splash instead of precision, so comparing the two is comparing different jobs. Assertion narrowed to non-arc weapons, with that reasoning recorded rather than the comparison quietly widened until it passed. Separately, `_bipod_blocks_firing()` read velocity without checking whether a bipod was actually down — correct in the one place it was called (which checked first), wrong as a function, and it would have answered "yes, blocked" for every stowed rifle. Now self-guarding.

**New test** (`run_tests.gd`): `test_anti_materiel_rifle_is_wired_and_trades_real_capability` — all six registration points, the per-shot claim against every direct-fire weapon, both tweak-plumbing traps, `optic_power` buying reach but provably *not* damage, the bipod being visible when deployed, `barrel_length` moving exactly one part, and the full bipod trade including that it fails **open** with no owning vehicle (a Test Range dummy or a building-mounted rifle has no locomotion, and silently never firing there would be worse than the mechanic is worth).

**Verified** with real non-headless renders (`scratch/capture_new_weapons.gd`): the M230's chain housing and yoke read correctly, and the rifle at `barrel_length` 2.2 grows only its tube while the brake travels to the new tip and the breech and sensor pod keep their authored proportions.

---

## 2026-07-31 — Design Lab parts bin rebuild, structural pieces rebuilt, part material roles, napalm mortar fix

Direct request from Chris, five parts: the napalm mortar's barrel looked upside down; sort the modules into more distinct groups, light-to-heavy within each; the structural modules "aren't really at the same standard at all" and should be resizable/draggable like hull-builder primitives; rearchitect the Design Lab menu for usability, researching how comparable assembly/sculpting tools do it; and work out what can be done to make weapon modules look better - "different materials on different parts, or better textures... Both? Other options?" (answer: both, plus a third thing neither of those names - see below).

**Napalm mortar (`visual_builder.gd`).** `ElevationPivot` used `deg_to_rad(-55.0)` where artillery and the mortar array both use a positive angle. The parts are authored with the bore along -Z and a positive X rotation pitches -Z *up*, so the negative sign pitched the whole assembly nose-down through the deck and put the flared muzzle underneath the breech - which is what read as the barrel being fitted upside down. Now `NAPALM_ELEVATION_DEG`, positive, with a test asserting the muzzle ends up above the breech rather than just checking the sign.

**Parts bin regrouped (`module_catalog.gd`, `parts_menu.gd`).** `category` is a mechanical classification - it tells the placer which gizmo handles to show - and 25 of the catalog's entries are `category: "weapon"`, so the Modules tab was one undifferentiated wall of buttons. Added `MODULE_ROLES`, a *browsing* classification (Direct-Fire Guns / Energy & Electromagnetic / Indirect Fire / Missiles / Point Defense / Deployables / Armor / Power / Support / Structural), kept **in the catalog rather than in the UI script** for the reason `HULL_MODDING_PLAN.md` 4c gives for hulls: a grouping table living in a UI file means a modded part needs a UI code change to appear anywhere sensible. Hulls likewise split off `is_foundation` + weight class (the old 2-way split meant one 22-entry "Vehicle" drawer), locomotion off its own `traits` array. Everything sorts **light to heavy** inside its group, with the weight shown inline on each row - sorting by a stat you can't see is not sorting.

**Parts bin rearchitected.** Research across KSP2's VAB, Trailmakers and Besiege turns up the same two complaints every time, and they are not the ones the grouping fixes. First: **finding** a specific part is the pain point, not browsing for one - in each community the answer was an external parts-list tool or a filter mod. Grouping helps browsing; only search helps retrieval, and they are different tasks. So: a search box that filters across all three tabs at once, force-opens the surviving drawers (making you click a header to see the thing you just searched for defeats the point), hides empty groups and fully restores on clear. Second: **discoverability** - controls documented only in an external guide are controls most players never find. These buttons are drag sources that look exactly like buttons you click, and nothing anywhere said so; the bin now states it. Plus count badges on collapsed headers, so a shut accordion doesn't make "which drawer do I look in" a coin flip.

**Structural pieces rebuilt (`build_structural.py`, `visual_builder.gd`, `gizmo_3d.gd`, `blueprint_manager.gd`).** These six are the only modules the player can scale freely on all three axes at once, and they're meant to be stretched hard - which rules out the approach every other part family uses (one authored `.glb`, scaled), because a bolt head scaled 4x along one axis is a smear. Split instead: the **body** stays procedural and re-tessellates, the **detail** is 10 new authored hardware parts instanced at their true authored size with no scaling. Stretch a girder and you get more splice collars, not longer ones. Paired with **scale isolation** - the resize travels as a `struct_scale` meta and the body is rebuilt, `node.scale` stays at ONE, exactly as the hull does - which is what makes them behave like hull-builder primitives. Both colliders (layer-2 click target, layer-16 mounting surface) and the clipping AABB are driven by hand to match, and the value persists through save/load.

**Two latent bugs found while in there, neither reported:** `structural_wedge` has *never* been a wedge - `_build_wedge_mesh()` declared eight vertices and then put the top four at full size on all axes, i.e. built a plain box, with comments describing an "apex" and a "top ridge" the geometry never had. Rewritten as a real glacis (flat-shaded, since it's folded plate). And the wedge body was offset +y/2 from a mesh that already spans 0..y, so it floated half its own height off whatever it was bolted to.

**Part material roles (`part_materials.gd`, new).** The honest answer to "different materials, or better textures?" is that neither was the actual problem. Every bolt-on part reached the screen through a `StandardMaterial3D` with exactly one property set - `albedo_color` - leaving Godot's defaults of metallic 0.0 / roughness 1.0, which is the PBR description of matte plastic. A barrel, a lens, an ammo drum and a rubber tyre had *identical surface response* and differed only in how dark their paint was. The parts were differentiated by geometry and by colour but not by **substance**. So: nine roles (steel/painted/gunmetal/scorched/brass/optics/rubber/ceramic/energized), each with its own metallic/roughness/base colour plus a `tint` weight controlling how much of the caller's colour survives - a barrel stays gunmetal on a red gun, a painted housing takes the red in full. Roles resolve from the authored **filename**, which is how ~190 existing parts pick one up with no call-site edits: they're already named after what they are. Texture is procedural triplanar noise (the authored meshes have no usable UVs and hand-unwrapping 190 procedurally-generated parts is not a real option), at two frequencies doing two jobs - fine on roughness so a flat face stops reading as a decal, coarse on detail-albedo so paint reads as sprayed onto metal.

**The material change forced a real bug fix it didn't cause.** Materials are shared per role+tint, because `bake_module_visual()` merges a battle module's meshes grouped by material *identity* and per-part materials would silently defeat the merge. But `check_all_clipping()` wrote `albedo_color`/emission straight onto each mesh's `material_override`, so one clipping module would have repainted every part in the scene sharing that role. Worse, the pre-existing "not clipping" branch flattened *every* mesh in a module to its catalog colour on every pass - and that pass runs on every placement, drag, rotation and tweak, so the per-part colours the builders already assigned never survived long enough to be seen at all. Now swaps to one shared red material and restores the part's own.

**One wrong turn worth recording.** The finer drawers plus a search bar tipped the Hulls tab into a genuine layout overflow, because a Container propagates its children's minimum size upward and `clip_contents` does not change that - a *collapsed* drawer was still demanding its full expanded height. Fixed by wrapping the button list in a plain `Control` (no such propagation). The audit then flagged the clip wrapper itself, and the obvious fix - exempt anything with `clip_contents` - was wrong: `ScrollContainer` sets that flag internally, so it would have silently exempted every ScrollContainer in the project, which is the exact node type the real overflow bug this tool was written for lived in. Caught by `test_ui_audit_has_real_teeth`, which is precisely what that suite exists for. Replaced with an explicit `ui_audit_clip_ok` meta - an opt-out has to be stated on purpose.

**New tests** (`run_tests.gd`): `test_module_roles_group_and_sort_the_parts_menu` (grouping off catalog fields only, light-to-heavy, nothing dropped, search filters + opens + restores across all tabs); `test_structural_pieces_resize_without_smearing_detail` (all six isolate the resize, hardware never scales *non-uniformly*, detail density rises with size, both colliders track); `test_structural_resize_survives_a_blueprint_round_trip`; `test_part_material_roles_differentiate_surfaces` (roles differ in surface response, not just colour; materials genuinely shared for the batch merge; a built weapon carries several); `test_clipping_highlight_does_not_corrupt_shared_materials`; `test_napalm_mortar_tube_points_upward`. `test_hull_modding_parts_menu_two_buckets` updated for the new hull grouping - the property under test (assignment computed from the hull's own fields, no per-type_id table in the UI) is unchanged.

**Verified** with real non-headless renders, not inferred: `scratch/capture_structural.gd` for the six pieces and a materials check, `scratch/capture_parts_menu_redesign.gd` for the bin. Those renders are what caught the dome being built at double height (Godot's `SphereMesh.is_hemisphere` treats `height` as the hemisphere's full height, not the source sphere's diameter - so it swallowed its own hatch and vision blocks whole), the ammo drum reading as polished trumpet brass, and the slab's edge beads reading as two enormous logs bolted to a thin plate.
...
**Shipped:**
- **1.3 item 3 — AI defends its HQ.** `enemy_ai.gd._defend_hq_if_under_attack()`, checked on the existing 8s structure-check timer: compares the enemy HQ's hp against the last reading, and if it dropped, queues the cheapest legal defense entry from `enemy_roster` (capped at 3 AI-placed defenses so a prolonged siege doesn't spiral into unbounded turret spam). Placement reuses `_find_ai_build_position()`'s default HQ-centered anchor with the defense's real 28m leash - "near the HQ" falls straight out of C3's existing adjacency rule. Needed a real data fix alongside the code: `res://data/enemy/` had **zero** defense (foundation-hull) blueprints at all, so the feature would have been correct code with nothing to build - added `data/enemy/gatling_pillbox.json`, mirroring the player's own default loadout entry.
- **1.3 item 4 investigated, not built.** "Expand toward an unclaimed resource cluster" runs into a real constraint: C3's buildable-area rule requires any new building to sit within adjacency of an *existing* friendly anchor, and there's no "found a new base" primitive at all. Rather than fake it with a same-base second refinery that wouldn't actually answer the roadmap's own wording, this stays open pending a design decision on a forward-base mechanic.
- **1.6 E2 — sell + repair.** Two new build-bar toggles (🔧 Repair / 💰 Sell), OpenRA's "armed cursor mode" convention - the next left-click on a friendly building performs the action. `building.gd` gained `build_cost_metal`/`build_cost_crystal` (static `PREFAB_STATS` lookup for prefab kinds, `blueprint_cost()` for a `defense`, since defense cost varies per design) so sell/repair share one formula regardless of building shape. `skirmish.sell_building()` refunds 50% of cost scaled by current hp fraction via a new `building.die(spawn_debris = false)` - selling isn't dying in combat, so no debris particles, but the same `died` signal still fires (E3's cancel-on-death and the C1 navmesh-hole rebake both key off it). `_process_repairs()` heals 7 HP per real 0.96s tick, drawing real metal/crystal per step (each resource repairs against its own share of the original build cost), auto-stopping at full HP.
- **1.6 E3 — cancel-unbuildable.** `_on_manufactory_died()` (wired into every manufactory's `died` signal) fires `production.cancel_unbuildable_items(team, tier)` the instant a tier's *last* live manufactory dies, refunding every queued item in that line via the same partial-refund formula `cancel()` already uses per item - `production_queue.gd`'s own `tick()` comment had documented this exact gap (a finished job with nowhere to spawn from just got silently dropped). Losing one of *two* live manufactories of a tier correctly leaves the queue alone. D2's existing per-tick `has_factory_of_tier()` button-greying already covered the "tech-tree greying" half of this chunk - no separate owned-kind-count cache needed.
- **Phase 2 A1 — environment/post-processing.** All 4 scenes with their own `WorldEnvironment` (`Skirmish`, `Battlefield`, `MainLab`, `HullBuilder`) previously shared an identical bare-minimum `Environment` (sky + Filmic tonemap, nothing else). Each now gets ACES tonemap, a modest bloom gated by `glow_hdr_threshold` so only genuinely emissive material blooms (not general geometry), and SSAO for contact shadowing under flush-mounted parts - one shared tuned value set across all 4 scenes since they're the same rendering pipeline.

**A real, unrelated incident during this session, not caused by this work:** partway through, `run_tests.gd` was found mid-corrupted by a concurrent process (Chris, in a separate session) removing `interceptor_hull` and its assets - a truncated `print()` call, an orphaned indentation block, and several dangling test registrations pointing at now-deleted functions, all of which left the suite unable to parse at all. Confirmed with Chris this was expected/in-progress work, not something to revert; once he confirmed the removal itself was finished, fixed the leftover syntax breakage directly (closed the truncated print, fixed the indentation, dropped registrations for the 2 genuinely-deleted test functions, and restored registrations for 2 functions that still existed but had lost their call sites as collateral damage) rather than reverting any of his actual content changes. Cross-checked with a scripted call-vs-definition diff (every `_run_suite(test_X, ...)` against every `func test_X()`) to make sure nothing else was left dangling, plus an increment-balance check (`_total_suites += 1` count vs. actual registered call count) that caught one duplicate leftover increment. Real lesson for next session: a concurrent edit to a shared file mid-session is a live hazard this project's tooling doesn't guard against - worth checking `git status`/parse-checking before trusting a "the suite is red" signal is about your own change.

**New tests** (`run_tests.gd`): `test_1_3_ai_places_a_defense_when_its_hq_takes_damage`; `test_e2_selling_a_building_refunds_health_scaled_50_percent`; `test_e2_repair_heals_over_real_ticks_and_spends_real_resources`; `test_e3_losing_last_manufactory_of_a_tier_refunds_its_queued_items`; `test_e3_losing_one_of_two_manufactories_of_a_tier_does_not_cancel_the_queue`.

**Verified:** full suite 159/160 (up from 158) via `.\run_tests.ps1`/`run_tests.sh` end to end (reimport + suite) - the one failure is the pre-existing, deliberately-left-failing `test_target_dummies_actually_take_damage_in_test_range` flake, documented since 2026-07-21 and again in `UNIFIED_ROADMAP.md` 0.4, unrelated to this session's changes.

**Not done this pass:** 1.2 (playtest - Chris's job), 1.5's C2 (debug HUD gating - still deliberately deferred until 1.2's testing happens), 1.3 item 4 (needs the forward-base design decision above). Phase 2's remaining chunks (A4, B1, C4, A2, B2, G, A3) are next per the roadmap's own recommended order.
